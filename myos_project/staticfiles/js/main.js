...
function buildCommandPaletteItems() {
  ...
  const shortcuts = [
    {
      ...
      action: () => {
        navigateInApp(PAGE_ROUTES.dashboard);
      },
    },
    {
      label: 'Open Goals',
  ...
  return [...quickItems, ...shortcuts, ...navItems];
}
...
function renderCommandPaletteOverlay(query = '') {
  ...
  list.innerHTML = uiState.commandItems.map((item, index) => `
    <button class="command-item${index === 0 ? ' active' : ''}" type="button" data-command-index="${index}">
      <span>
        <div class="command-item-title">${item.label}</div>
        <div class="command-item-meta">${item.meta}</div>
      </span>
      <i class="${item.icon}"></i>
    </button>
  `).join('');
    ...
    panel.innerHTML = uiState.commandItems.map((item, index) => `
      <button class="search-panel-item${index === 0 ? ' active' : ''}" type="button" data-command-index="${index}">
        ...
        <span class="search-panel-copy">
          <span class="search-panel-title">${item.label}</span>
          <span class="search-panel-meta">${item.meta}</span>
        </span>
...
function renderSearchResults(results, container, query) {
  ...
  container.innerHTML = results.map((result) => `
    <a class="search-result-item" href="${escapeHtml(result.url || '#')}">
      <span class="search-result-icon"><i class="fas ${escapeHtml(result.icon || 'fa-circle')}"></i></span>
      <span class="search-result-body">
        <span class="search-result-title">${escapeHtml(result.title || '')}</span>
        <span class="search-result-preview">${escapeHtml(result.preview || '')}</span>
      </span>
      <span class="search-result-type">${escapeHtml(result.group || result.type || '')}</span>
    </a>
  `).join('');
  container.classList.remove('hidden');
}
...
function runCounterAnimations(root = document) {
  root.querySelectorAll?.('.metric-value, .college-stat-value, .media-stat-value, .finance-amount').forEach((el) => {
    ...
    const format = (value) => `${prefix}${value.toLocaleString(undefined, {
      minimumFractionDigits: decimals,
      maximumFractionDigits: decimals,
    })}${suffix}`;
